...
// ExtractCliqueTree returns a clique list and adjacency list
func ExtractCliqueTree(T *Tree, iphi []int) (cliques [][]int, adj [][]int) {
	n := len(iphi)
	k := n - len(T.P) + 1
	...
	// complete adjacency list with every node adjacent to its father
	adj = children
	for i := 1; i < len(T.P); i++ {
		adj[i] = append(adj[i], T.P[i])
	}
